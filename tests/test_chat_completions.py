...
async def test_chat_completion_with_model_override(test_client_with_model, monkeypatch):
    ...
    async def mock_post(*args, **kwargs):
        ...
        request_body["model"] | should.equal("gpt-4-test")  # Should always use config model
...
async def test_chat_completion_content_length(test_client_blank_model, monkeypatch):
    ...
    async def mock_post(*args, **kwargs):
        ...
        if content_length:
            actual_length = len(content) if isinstance(content, bytes) else len(str(content).encode())
